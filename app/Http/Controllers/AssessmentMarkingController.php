...
class AssessmentMarkingController extends Controller
{
    public function GenerateStudentMarkingPage($student_id, $writing_task_id){
        ...
        $student_assessed_level = DB::table('classes_students')->join('assessed_level_labels', 'classes_students.student_assessed_label_id', 'assessed_level_labels.assessed_level_label_id')->select('assessed_level_labels.assessed_level_label')->where('classes_students.students_student_Id', '=', $student->student_Id)->get()->toArray();
        ...
        return view('assessment-marking', ['rubrics' => $rangeAsScriibiValue, 'skillCards' => $skillCards, 'firstName' => $student->student_First_Name, 'lastName' => $student->student_Last_Name, 'student_id' => $student->student_Id, 'writting_task_id' => $writing_task_id, 'status' => $status[0]->status, 'assessed_level' => $student_assessed_level, 'assessed_level_scriibi_id' => $student->rubrik_level]);
    }
